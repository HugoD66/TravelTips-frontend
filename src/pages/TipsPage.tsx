  ...
  useEffect(() => {
    const fetchTipDetails = async () => {
      ...
      try {
        ...
        setError(
          "Failed to fetch tip details. Please check the console for more information."
        );
      }
    };
      ...
      {tip ? (
        <>
          <div className="tip-header">
            ...
            <p className="tip-creation-date">
              Créé le : {new Date(tip.createdAt).toLocaleDateString()}
            </p>
          </div>
          <div className="tip-img">
            {pictureList.filter((picture) => picture.idTips!.id === tip.id)
              .length > 0
