...
export const getLastestItinerary = async () => {
  return fetch("http://172.16.70.192:4000/itinerary/latest", {
    method: "GET",
  })
    .then((response) => {
      if (!response.ok) {
...
export const getItineraryUser = (id: string, token: string) => {
  return fetch(`http://172.16.70.192:4000/itinerary/users/${id}`, {
    method: "GET",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
export const updateItinerary = (itineraryId: string, itinerary: ItineraryModel, token: string) => {
  return fetch(`http://172.16.70.192:4000/itinerary/${itineraryId}`, {
    method: "PATCH",
    body: JSON.stringify(itinerary),
    headers: {
      "Content-Type": "application/json",
